#  app/python/ai_processing/utils/data_converters.py

import json
import re
from app.python.ai_processing.utils.data_handler import load_data
...
def bio_to_offset(nlp, text, labels):
    ...
    # print(f"text: {text} ")
    all_tokens, all_labels = add_space_to_tokens(
        tokens, labels, no_space_entities, punctuations
    )
    ...
    if len(all_tokens) != len(all_labels):
        raise ValueError(
            f"Token and label length mismatch. Tokens: {len(all_tokens)}, Labels: {len(all_labels)}"
        )
    ...
    for i, (word, label) in enumerate(zip(all_tokens, all_labels)):
        ...
        if label.startswith("B-"):
            ...
            if (
                ...
                and (not next_label.startswith("I-"))
            ):
...
def convert_biluo_to_tokens_and_labels(text, all_tokens, all_labels):
    ...
    text_tokens = re.findall(
        r"\b(?:part|full|time)-\w+\b|\b\w+(?:,\w+)*\b|[^\w\s]", text, re.IGNORECASE
    )
...
def custom_offsets_to_biluo_tags(spans, text):
    ...
    new_tokens = [
        tag.split(", Label: ")[0].replace("Token: ", "").strip() for tag in new_tags
    ]

    tokens_with_spaces, _ = add_space_to_tokens(
        new_tokens, new_labels, no_space_entities, punctuations
    )
...
def align_biluo_tags(char_to_token_index, biluo_tags, document_text):
    ...
    for word in document_text:
        ...
        for _ in word:
            while (
                token_pos < len(char_to_token_index)
                and char_to_token_index[token_pos] == -1
            ):
                token_pos += 1
        ...
        if word_tags:
            if "JOB_COUNTRY" in word_tags[0]:
                ...
                final_tag = (
                    word_tags[0]
                    if word_tags[0].startswith("B-") or word_tags[0].startswith("U-")
                    else word_tags[-1]
                )
...
def convert_tokens_to_whole_word(doc, biluo_tags, spans, tokens_with_spaces):
    ...
    tags = align_biluo_tags(
        char_to_token_index, biluo_tags, [token.text for token in doc]
    )
