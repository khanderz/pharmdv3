...
def fix_entity_offsets(training_data, tokenized_output):
    ...
    for data in tokenized_output:
        ...
        for entity in entities:
            ...
            token_element = {"start": start, "end": end, "token": token}
            element.append(token_element)

        deconstructed_output.append({"text": text, "entities": element})
        ...
        if text == tokenized_text:
            ...
            while remaining_tokens:
                ...
                for tokenized_entity in tokenized_entities:
                    ...
                    print(
                        f"tokenized_entity: {token_text}, token_start: {token_start}, token_end: {token_end}"
                    )
                ...
                if not token_found:
                    print(
                        "No matching token found for remaining_tokens:",
                        remaining_tokens,
                    )
                    break
            ...
            if not remaining_tokens:
                ...
                if tokenized_sequence:
                    ...
                    print(
                        f"{BLUE}Not all tokens processed, remaining tokens: {remaining_tokens} {RESET}"
                    )

            print(
                f"{BLUE}Updated entity: start {entity['start']}, end {entity['end']}{RESET}"
            )

            updated_training_data.append({"text": text, "entities": entities})
