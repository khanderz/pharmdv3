...
def fetch_healthcare_domains_keywords():
    ...
    healthcare_domains_keywords = {
        domain['value'].lower(): [domain['value'].lower()] + [alias.lower() for alias in domain['aliases']]
...
def classify_healthcare_domain(healthcare_domains_keywords, page_text):
    ...
    for domain, keywords in healthcare_domains_keywords.items():
        ...
        for keyword in keywords:
            # Use regex to match whole words exactly, ensuring standalone matches
        ...
        if matched_keywords:
            ...
            print(f"Matched domain '{domain.title()}' with keywords {matched_keywords} in page text.")
...
def update_google_sheet_with_domains(sheet_id, range_name):
    ...
    # Loop through all rows in master_data
    for index, row in master_data.iterrows(): 
        ...
        # Remove any previously matched domains that no longer match
        ...
        if updated_domains != existing_domains:
            ...
            print(f"Updated {row['company_name']} with matched domains: {updated_domains}")
        else:
