...
def fetch_ats_keywords():
    ...
    try:
        ...
        ats_keywords = {ats['ats_type_name']: ats['ats_type_code'].lower() for ats in ats_data}
        return ats_keywords
    except requests.RequestException as e:
...
def extract_and_classify_info(company_name, company_url):
    ...
    try:
        ...
        healthcare_domains, specialties = classify_healthcare_domain_specialty(about_content)
    except requests.RequestException as e:
...
def update_with_ai_info(filled_master_data):
    for index, row in filled_master_data.iterrows():
        if pd.isna(row['company_ats_type']) or pd.isna(row['healthcare_domain']) or pd.isna(row['company_specialty']):
            company_url = row['company_url']
            ats_type, healthcare_domains, specialties = extract_and_classify_info(row['company_name'], company_url)
            
            if ats_type:
                filled_master_data.at[index, 'company_ats_type'] = ats_type
            if healthcare_domains:
                filled_master_data.at[index, 'healthcare_domain'] = ', '.join(healthcare_domains)
            if specialties:
                filled_master_data.at[index, 'company_specialty'] = ', '.join(specialties)
            
            print(f"Updated {row['company_name']} with ATS type: {ats_type}, domains: {healthcare_domains}, specialties: {specialties}")
