...
def fill_missing_values_with_hubspot(master_data, hubspot_data):
    ...
    for _, hubspot_row in hubspot_data.iterrows():
        ...
        if not matching_master_row.empty:
            ...
            for column in ['company_url', 'is_public', 'logo_url', 'year_founded', 'linkedin_url', 'company_description', 'company_size']:
                if pd.isna(master_data.at[index, column]) or master_data.at[index, column] == '':
