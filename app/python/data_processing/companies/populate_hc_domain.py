...
def process_and_update_sheet(credentials_path, sheet_id,  master_active_data, master_linkedin_issues_sheet_name):
    if "company_description" not in master_active_data.columns or "healthcare_domain" not in master_active_data.columns:
    ...
    for index, row in master_active_data.iterrows():
        ...
        if not company_description:
            print(f"{RED}Skipping row {index+1} (no description provided for {company_name}).{RESET}")
            continue
        ...
        try:
            ...
            print(f"{BLUE}HC domain found for {company_name}: {healthcare_domains_str}{RESET}")
