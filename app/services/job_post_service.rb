...
class JobPostService
  def self.extract_currency_from_text(job_post)
    ...
    if currency_mappings.empty?
      puts 'Currency mappings are empty. Check the database entries for JobSalaryCurrency.'
      return nil
    end

    currency_symbol = currency_mappings.keys.find do |symbol|
      symbol && combined_text.include?(symbol)
    end

    if currency_symbol
      currency_code = currency_mappings[currency_symbol]
  ...
  def self.extract_salary_range(job_post)
    ...
    currency_data = currency_mappings.map do |sign, code|
      if sign && code && !sign.empty? && !code.empty?
        "#{sign}:#{code}"
      else
        puts "Warning: Skipping invalid currency pair '#{sign}:#{code}'"
        nil
      end
    end.compact.join(',')
