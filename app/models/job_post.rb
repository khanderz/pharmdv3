...
class JobPost < ApplicationRecord
  ...
  has_paper_trail has_many :adjudications, as: :adjudicatable, dependent: :destroy has_many :job_post_cities, dependent: :destroy has_many :cities, through: :job_post_cities has_many :job_post_states, dependent: :destroy has_many :states, through: :job_post_states has_many :job_post_countries, dependent: :destroy has_many :countries, through: :job_post_countries belongs_to :job_commitment, optional: true belongs_to :job_setting belongs_to :department belongs_to :team belongs_to :company belongs_to :job_role belongs_to :job_salary_currency, optional: true belongs_to :job_salary_interval, optional: true validates :job_title, presence: true validates :job_url, uniqueness: true validates :job_salary_min, :job_salary_max, numericality: { greater_than_or_equal_to: 0 }, allow_nil: true  # Shared methods -----------------------------------------------------------
