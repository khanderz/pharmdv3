...
class Company < ApplicationRecord
  has_paper_trail ignore: [:updated_at] belongs_to :ats_type belongs_to :company_size, optional: true belongs_to :funding_type, optional: true # Remove direct references to city, state, and country in favor of join tables has_many :company_cities, dependent: :destroy has_many :cities, through: :company_cities, dependent: :destroy has_many :company_states, dependent: :destroy has_many :states, through: :company_states, dependent: :destroy has_many :company_countries, dependent: :destroy has_many :countries, through: :company_countries, dependent: :destroy has_many :company_domains has_many :healthcare_domains, through: :company_domains has_many :adjudications, as: :adjudicatable, dependent: :destroy has_many :job_posts, dependent: :destroy has_many :company_specializations has_many :company_specialties, through: :company_specializations validates :company_name, presence: true, uniqueness: true validates :linkedin_url, uniqueness: true, allow_blank: true  def self.seed_existing_companies(company, row, ats_type, country)
      changes_made = false
