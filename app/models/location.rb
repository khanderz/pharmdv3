class Location < ApplicationRecord
    ...
    class << self
        def find_or_create_by_name_and_type(name, type, attributes = {})
          ...
          location = where('LOWER(name) = ? OR ? = ANY(aliases)', normalized_name, normalized_name).find_by(type: type)
        ...
        def find_by_name_and_type(name, type)
          normalized_name = name.strip.downcase
          where('LOWER(name) = ? OR ? = ANY(aliases)', normalized_name, normalized_name).find_by(type: type)
        end
      end
  end
